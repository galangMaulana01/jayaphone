...
const money = (value?: number): string => value ? `Rp ${value.toLocaleString("id-ID")}` : NOT_SET;
export default function ApprovalCodPage(): JSX.Element {
  const { showToast } = useToast(); const cabangTz = useCabangTimezones(); const [selected, setSelected] = useState<CODRequest | null>(null); const [rejecting, setRejecting] = useState<CODRequest | null>(null); const [reason, setReason] = useState(""); const [draft, setDraft] = useState<UnitDraft>(EMPTY_DRAFT); const [hargaJual, setHargaJual] = useState(""); const [foto, setFoto] = useState<string[]>([]);
