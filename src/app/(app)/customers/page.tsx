...
function CustomersPageInner(): JSX.Element {
  ...
  const { items, loading, loadingMore, error, total, hasMore, reload: load, loadMore } = usePaginatedApiList<Customer>(
    (skip, limit) => Api.customer.list({ status: status || undefined, skip, limit }),
    [status],
  ...
  const create = async () => { if (!name.trim() || !contact.trim()) { showToast("Nama dan kontak wajib diisi", "error"); return; } if (name.trim().length > 100 || contact.trim().length > 100) { showToast("Nama dan kontak maksimal 100 karakter", "error"); return; } try { await Api.customer.create({ nama: name.trim(), kontak: contact.trim(), cabang: user?.cabang || "JYP" }); showToast("Customer berhasil ditambahkan"); setName(""); setContact(""); setAddOpen(false); await load(); } catch (e) { showToast(e instanceof Error ? e.message : "Gagal menambah customer", "error"); } };
