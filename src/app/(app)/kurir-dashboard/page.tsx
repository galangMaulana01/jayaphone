...
export default function KurirDashboardPage(): JSX.Element {
  ...
  return <div className="jp-page"><div><h1 className="jp-page-title">Dashboard Kurir</h1><p className="text-sm text-jp-muted dark:text-jp-muted-dark">Kelola tugas COD beli, jual, dan delivery</p></div>{loading ? <LoadingSkeleton numberOfRows={5}/> : error ? <ErrorState message={error} onRetry={load}/> : <>
    ...
    <div className="hidden md:block table-wrap overflow-x-auto rounded-jp-md"><table className="w-full text-xs"><thead className="tbl-head"><tr>{["Waktu","Tipe","Detail","Lokasi","Status","Aksi"].map((h) => <th key={h} className={`px-5 py-3.5 text-left font-medium ${h === "Aksi" ? "tbl-action-col" : ""}`}>{h}</th>)}</tr></thead><tbody>{items.length ? items.map((item) => <tr key={item.cod_id} className="tbl-row"><td className="whitespace-nowrap px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{formatDateTimeShort(item.created_at, resolveCabangTimezone(cabangTz, item.cabang))}</td><td className="px-5 py-4"><span className="badge">{item.type}</span></td><td className="px-5 py-4 font-medium">{item.product_name || item.trx_id || item.items?.map((x) => x.nama).join(", ") || "COD"}</td><td className="px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{item.delivery_address || item.location || NOT_SET}</td><td className="px-5 py-4"><CodStatusBadge status={item.status}>{labels[item.status] || item.status}</CodStatusBadge></td><td className="tbl-action-col px-5 py-4 whitespace-nowrap">{action(item)}</td></tr>) : <tr><td colSpan={6}><EmptyState message="Belum ada tugas COD" iconName="truckSvg"/></td></tr>}</tbody></table></div>
  </>}
