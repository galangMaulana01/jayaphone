...
export default function KurirDashboardPage(): JSX.Element {
  ...
  const visibleItems = useMemo(() => items.filter((item) => (!statusFilter || item.status === statusFilter) && `${item.product_name || ""} ${item.trx_id || ""} ${item.location || ""} ${item.delivery_address || ""}`.toLowerCase().includes(query.toLowerCase())), [items, query, statusFilter]);
  const taskAktif = items.filter((item) => !["selesai", "ditolak", "gagal", "terkirim", "transaksi_berhasil"].includes(item.status)).length;
  const menungguAksi = items.filter((item) => ["menunggu_kurir", "diterima", "kurir_menuju_lokasi", "sudah_bertemu_penjual", "sedang_diantar"].includes(item.status)).length;
  const selesaiBulanIni = items.filter((item) => ["selesai", "terkirim", "transaksi_berhasil"].includes(item.status)).length;
  ...
  return <div className="jp-page kurir-dashboard"><div><h1 className="jp-page-title">Dashboard COD</h1><p className="text-sm text-jp-muted dark:text-jp-muted-dark">Kelola tugas COD beli, jual, dan delivery cabang aktif.</p></div>{loading ? <LoadingSkeleton numberOfRows={5}/> : error ? <ErrorState message={error} onRetry={load}/> : <>
    <section className="grid grid-cols-3 gap-3"><div className="metric-card"><p className="label">Tugas Aktif</p><strong className="text-2xl">{taskAktif}</strong></div><div className="metric-card"><p className="label">Menunggu Aksi</p><strong className="text-2xl">{menungguAksi}</strong></div><div className="metric-card"><p className="label">Selesai Bulan Ini</p><strong className="text-2xl">{selesaiBulanIni}</strong></div></section><div className="jp-toolbar"><input className="field-control flex-1" placeholder="Cari tugas atau lokasi..." value={query} onChange={(event) => setQuery(event.target.value)} /><select className="field-control w-full sm:w-auto" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">Semua Status</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
    {/* Mobile: stacked cards so Lokasi/Status never end up scrolled off-screen like the table does on a phone. */}
    <div className="md:hidden space-y-3">{visibleItems.length ? visibleItems.map((item) => <div key={item.cod_id} className="panel rounded-jp-md p-4 space-y-2"><div className="flex items-center justify-between"><span className="badge">{item.type}</span><span className="text-[10px] text-jp-muted dark:text-jp-muted-dark">{formatDateTimeShort(item.created_at, resolveCabangTimezone(cabangTz, item.cabang))}</span></div><p className="text-sm font-medium">{item.product_name || item.trx_id || item.items?.map((x) => x.nama).join(", ") || "COD"}</p><div className="flex items-center justify-between gap-2 text-xs"><span className="text-jp-muted dark:text-jp-muted-dark">{item.location || item.delivery_address || NOT_SET}</span><CodStatusBadge status={item.status}>{labels[item.status] || item.status}</CodStatusBadge></div><div className="flex flex-wrap gap-2 pt-1">{action(item)}</div></div>) : <EmptyState message="Belum ada tugas COD" iconName="truckSvg"/>}</div>
    {/* Desktop: original table. */}
    <div className="hidden md:block table-wrap overflow-x-auto rounded-jp-md"><table className="w-full text-xs"><thead className="tbl-head"><tr>{["Waktu","Tipe","Detail","Lokasi","Status","Aksi"].map((h) => <th key={h} className={`px-5 py-3.5 text-left font-medium ${h === "Aksi" ? "tbl-action-col" : ""}`}>{h}</th>)}</tr></thead><tbody>{visibleItems.length ? visibleItems.map((item) => <tr key={item.cod_id} className="tbl-row"><td className="whitespace-nowrap px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{formatDateTimeShort(item.created_at, resolveCabangTimezone(cabangTz, item.cabang))}</td><td className="px-5 py-4"><span className="badge">{item.type}</span></td><td className="px-5 py-4 font-medium">{item.product_name || item.trx_id || item.items?.map((x) => x.nama).join(", ") || "COD"}</td><td className="px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{item.location || item.delivery_address || NOT_SET}</td><td className="px-5 py-4"><CodStatusBadge status={item.status}>{labels[item.status] || item.status}</CodStatusBadge></td><td className="tbl-action-col px-5 py-4 whitespace-nowrap">{action(item)}</td></tr>) : <tr><td colSpan={6}><EmptyState message="Belum ada tugas COD" iconName="truckSvg"/></td></tr>}</tbody></table></div>
  </>}
