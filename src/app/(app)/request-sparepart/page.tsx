...
export default function RequestSparepartPage(): JSX.Element {
  ...
  // Difilter jenis=repair — teknisi request part buat repair, bukan untuk
  // menjelajahi stok "dijual" milik toko yang tidak relevan buat mereka.
  ...
  return <div className="jp-page">
    ...
    {canCreate && <div className="space-y-2">
      ...
      {stokLoading ? <LoadingSkeleton numberOfRows={3}/> : <div className="table-wrap overflow-x-auto rounded-jp-md"><table className="w-full text-xs"><thead className="tbl-head border-b"><tr>{["ID","Nama","Kategori","Stok","Harga Modal","Aksi"].map((h) => <th key={h} className={`px-5 py-3.5 text-left font-medium ${h === "Aksi" ? "tbl-action-col" : ""}`}>{h}</th>)}</tr></thead><tbody>{visibleStok.length ? visibleStok.map((s) => <tr key={s.sp_id} className="tbl-row"><td className="px-5 py-4 font-mono text-jp-muted dark:text-jp-muted-dark">{s.sp_id}</td><td className="px-5 py-4 font-medium">{s.nama}</td><td className="px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{s.kategori}</td><td className={`px-5 py-4 font-semibold ${s.stok <= 0 ? "text-jp-danger dark:text-jp-danger-dark" : "font-mono text-jp-text dark:text-jp-text-dark"}`}>{s.stok} {s.satuan}</td><td className="px-5 py-4">{formatRupiah(s.harga_beli)}</td><td className="tbl-action-col px-5 py-4"><button type="button" className="btn-ghost" onClick={() => openRequestForm(s)}>Request</button></td></tr>) : <tr><td colSpan={6}><EmptyState message={stokQuery ? "Tidak ada sparepart yang cocok dengan pencarian" : "Belum ada sparepart di cabang Anda"} iconName="wrenchSvg"/></td></tr>}</tbody></table></div>}
    </div>}
