...
const EMPTY_FORM = { kat_kode: "", kondisi_kode: "", merk: "", tipe: "", storage: "", ram: "", warna: "", imei: "", imei2: "", tipe_sim: "", keamanan: "", speaker: "", lcd: "", kondisi_hp: "Mulus", battery: "100", harga_modal: "", harga_jual: "", garansi_toko: "7", catatan: "", keluhan: "" };
  ...
  return (
    <div className="space-y-3">
      ...
      {loading ? (
        ...
        <div className="max-h-64 space-y-1 overflow-y-auto rounded-xl border border-jp-border p-1.5 dark:border-jp-border-dark">
          {spareparts.map((sp) => {
            ...
            return (
              <button
                ...
                onClick={() => toggle(sp)}
                className={`flex w-full items-center justify-between rounded-lg p-2.5 text-left transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${isSelected ? "bg-jp-surface-subtle dark:bg-jp-surface-subtle-dark" : "hover:bg-jp-surface-subtle/60 dark:hover:bg-jp-surface-subtle-dark/60"}`}
              >
      ...
      {cart.length > 0 && (
        <div className="space-y-1.5">
          ...
          {cart.map((item) => (
            <div key={item.sp_id} className="flex items-center justify-between rounded-lg border border-jp-border bg-jp-surface-subtle p-2.5 dark:border-jp-border-dark dark:bg-jp-surface-subtle-dark">
              <span className="text-xs font-medium text-jp-text dark:text-jp-text-dark">{item.nama}</span>
              <span className="flex items-center gap-1.5">
                <button type="button" onClick={() => changeQty(item.sp_id, -1)} className="flex h-6 w-6 items-center justify-center rounded-lg bg-jp-surface text-xs dark:bg-jp-surface-dark">−</button>
