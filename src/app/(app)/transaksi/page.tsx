...
export default function TransaksiPage(): JSX.Element {
  ...
  return <div className="jp-page">
    ...
    {loading ? <LoadingSkeleton numberOfRows={6} /> : error ? <ErrorState message={error} onRetry={load} /> : <div className="table-wrap overflow-hidden rounded-jp-md">
      <div className="overflow-x-auto"><table className="w-full text-xs"><thead className="tbl-head border-b"><tr>{["ID Trx","Unit","Kasir","Harga Jual","Modal","Profit","Poin Dipakai","Waktu","Detail"].map((h) => <th key={h} className={`px-5 py-3.5 text-left font-medium ${h === "Detail" ? "tbl-action-col" : ""}`}>{h}</th>)}</tr></thead><tbody>
        {items.length ? items.map((item) => <tr key={item.trx_id} className="tbl-row"><td className="px-5 py-4 font-mono text-jp-muted dark:text-jp-muted-dark">{item.trx_id}</td><td className="px-5 py-4"><p className="font-medium">{item.unit_label}</p><p className="text-[10px] text-jp-muted dark:text-jp-muted-dark">{item.unit_id || "Sparepart"}</p></td><td className="px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{item.kasir}</td><td className="px-5 py-4 whitespace-nowrap">{formatRupiah(item.harga_jual)}</td><td className="px-5 py-4 whitespace-nowrap text-jp-muted dark:text-jp-muted-dark">{formatRupiah(item.harga_modal)}</td><td className="px-5 py-4 whitespace-nowrap font-semibold font-mono"><span className={item.profit < 0 ? "text-jp-danger dark:text-jp-danger-dark" : "text-jp-text dark:text-jp-text-dark"}>{formatRupiah(item.profit)}</span>{item.profit < 0 && <span className="badge badge-sold ml-2">Rugi</span>}</td><td className="whitespace-nowrap px-5 py-4 font-mono text-jp-muted dark:text-jp-muted-dark">{item.poin_dipakai || 0} poin</td><td className="whitespace-nowrap px-5 py-4 text-jp-muted dark:text-jp-muted-dark">{formatDateTimeShort(item.waktu, resolveCabangTimezone(cabangTz, item.cabang))}</td><td className="tbl-action-col px-5 py-4"><button type="button" className="btn-ghost" onClick={async () => { try { setSelected((await Api.transaksi.detail(item.trx_id)).data); } catch (e) { showToast(e instanceof Error ? e.message : "Gagal memuat detail", "error"); } }}>Detail</button></td></tr>) : <tr><td colSpan={9}><EmptyState message="Belum ada transaksi" iconName="transaksiSvg" /></td></tr>}
