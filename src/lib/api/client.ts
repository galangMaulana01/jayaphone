...
/** DELETE a previously uploaded image using the legacy multipart contract. */
export async function deleteUploadedImage(publicId: string): Promise<unknown> {
  if (!publicId.trim()) throw new ApiError(400, "public_id wajib diisi");
  const formData = new FormData();
  ...
  let httpResponse: Response;
  try {
    httpResponse = await fetch(buildFullUrl("/upload/image"), {
      method: "DELETE",
      headers: authToken ? { Authorization: `Bearer ${authToken}` } : {},
      body: formData,
    ...
    clearTimeout(timer);
    if (error instanceof DOMException && error.name === "AbortError") throw new ApiError(0, "Penghapusan gambar timeout, coba lagi.");
    throw new ApiError(0, "Tidak dapat terhubung ke server.");
  }
  clearTimeout(timer);
  const parsedBody = (await httpResponse.json().catch(() => ({}))) as { data?: unknown; detail?: unknown; message?: unknown };
  if (!httpResponse.ok) {
    if (httpResponse.status === 401) handleUnauthorized();
    throw new ApiError(httpResponse.status, coalesceErrorDetail(parsedBody.detail ?? parsedBody.message ?? "Penghapusan gambar gagal"));
  }
  return parsedBody.data ?? parsedBody;
}
