...
import { requestJson, uploadImageFile, uploadImageFiles } from "./client";
import type {
  ApiEnvelope,
  ActivityLog,
  AuthenticatedUser,
...
function buildQueryString(paramsObject: Record<string, unknown> | undefined): string {
...
const karyawan = {
  ...
  stats: (karyawanId: string, params?: { date_from?: string; date_to?: string; hari?: number }) =>
    requestJson<ApiEnvelope<unknown>>("GET", `/karyawan/${karyawanId}/stats${buildQueryString(params)}`),
...
const requestSparepart = {
  list: (params?: { status?: string }) =>
    requestJson<ApiEnvelope<unknown[]>>("GET", `/request-sparepart${buildQueryString(params)}`),
  create: (body: {
    tipe: string;
    service_id?: string;
    sp_id?: string;
    ...
    cabang: string;
    product_link?: string;
  }) => requestJson<ApiEnvelope<unknown>>("POST", "/request-sparepart", body),
  respond: (reqId: string, body: { status: string; estimasi_tiba?: string; catatan?: string }) =>
    requestJson<ApiEnvelope<unknown>>("PATCH", `/request-sparepart/${reqId}/respond`, body),
  approve: (reqId: string, body: { harga_jual: number; status: string; catatan?: string }) =>
    requestJson<ApiEnvelope<unknown>>("PATCH", `/request-sparepart/${reqId}/approve`, body),
};
...
const cod = {
  ...
  kurirMonitoring: (params?: { cabang?: string; date_from?: string; date_to?: string }) =>
    requestJson<ApiEnvelope<unknown[]>>("GET", `/cod/kurir/monitoring${buildQueryString(params)}`),
};
...
const influencer = {
  ...
  sync: () => requestJson<ApiEnvelope<unknown>>("POST", "/influencer/sync"),
};

const ownerInfluencer = {
  dashboard: () => requestJson<ApiEnvelope<unknown>>("GET", "/influencer/owner/dashboard"),
